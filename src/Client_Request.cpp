...
/**
 * @brief Handles the client's request by validating, routing, and processing
 * it.
 *
...
void Client::handleClientRequest() {
  ...
  std::cout << "[DEBUG] Start handling request for client on socket: "
            << _socket << std::endl;
  ...
  // now we create a location bloc by default so we always find a location
  // block.
  //  if (locationStatus == -1) // Location not found
  //  {
  //      sendErrorResponse(404, "Not Found: Invalid resource location");
  //      std::cerr << "[ERROR] Resource location not found: " << resource <<
  //      std::endl; return;
  //  }
  if (locationStatus == 1) // Redirect handled
  {
    std::cout << "[DEBUG] Redirect handled for resource: " << resource
              << std::endl;
    return;
  ...
  std::cout << "[DEBUG] Successfully handled request for client on socket: "
            << _socket << std::endl;
}
...
bool Client::_checkRequest() {
  ...
  if (!_checkFirstLine(ss) || !_checkAndGetHeaders(ss) ||
      !_checkAndGetPayload(ss))
    return false;
 ...
 *
 * @details Extracts the method, resource path, and HTTP version from the
 * request line. Validates the following:
...
bool Client::isUrlValid(const std::string &url) const {
  ...
  for (std::string::const_iterator it = decodedUrl.begin();
       it != decodedUrl.end(); ++it) {
 ...
 * @return A decoded URL with all percent-encoded characters replaced by their
 * original values.
 */
...
/**
 * @brief Matches the requested resource with server locations and updates root
 * path if applicable.
 *
 ...
 * @return -1 if no matching location is found, 1 if a redirect is handled, 0
 * otherwise.
 */
int Client::_checkLocation(std::string &root, std::string &resource,
                           size_t loopCount) {
  ...
  for (std::map<std::string, location_t>::const_iterator it = locations.begin();
       it != locations.end(); ++it) {
    if (resource.find(it->first) == 0 &&
        it->first.length() > matchedPrefixLength) {
  ...
  if (matchedLocation) {
    ...
    if (!matchedLocation->redirect_path.empty()) {
      sendRedirectResponse(ft_stoll(matchedLocation->redirect_err),
                           matchedLocation->redirect_path); // Redirect
      return 1;                                             // Redirect handled
    }
    // Check if the method is allowed
    if (std::find(matchedLocation->methods.begin(),
                  matchedLocation->methods.end(),
                  _method) == matchedLocation->methods.end())
      return -2;
    ...
    std::cout << "root : resource matched location " << root << " : "
              << resource << std::endl;
    return 0; // Successfully matched a location
  } // code added by tsanglar to debug the problem with a server bloc without
    // location bloc.
  else {

    // if no location bloc, a "by default" location bloc is created in
    // 'getDefaultLocation' and used during the handling of request.
    return 0;
  }
