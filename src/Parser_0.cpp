 ...
 *
 * This function tokenizes each line of the configuration file to get the server
 * configuration. Each token is a struct composed by:
 * - the line (a string)
 * - the type (an enum) : TK_EMPTY, TK_COMMENT, TK_SERVER, TK_LOCATION,
 * TK_CLOSE_BRACKET, TK_TOKEN
 */
 ...
 *
 * Each line from the config file is put as a 'struct token' in a
 * 'vectorTokensVector'.
 *
...
void Parser::_parseFile() {
  ...
  while (std::getline(this->_confFile, line)) {
    ...
    if (wordsVect.empty()) {
      ...
      tmpToken.value = line;
      this->_tokensVector.push_back(tmpToken);
    } else if ((wordsVect.size() == 2 && wordsVect[0] == "server" &&
                wordsVect[1] == "{") ||
               (wordsVect.size() == 1 && wordsVect[0] == "server{")) {
      tmpToken.type = TK_SERVER;
      tmpToken.value = line;
      this->_tokensVector.push_back(tmpToken);
    } else if ((wordsVect.size() == 2 && wordsVect[0] == "location" &&
                wordsVect[1][wordsVect[1].size() - 1] == '{') ||
               (wordsVect.size() == 3 && wordsVect[0] == "location" &&
                wordsVect[2] == "{")) {
      ...
      tmpToken.value = line;
      this->_tokensVector.push_back(tmpToken);
    } else { // the error_page directive needs to accept unlimited values on a
             // line -> any condition about the number of argument
 ...
 *
 * Check if there is a closing bracket for each blocks in the configuration
 * file.
 *
 */
 ...
 *
 * Each token is a struct containing a string (the line of the configuration
 * file) and an enum (its type). Each `TK_SERVER` token means the begin of a
 * server block. Therefore the following token are parsed to get this specific
 * Server configuration. Then check this configuration. Then use the
 * configuration to instantiate a 'Servers'
 *
 */
void Parser::_getConfigAndInitServers() {
  ...
  for (; this->_nbLine < this->_tokensVector.size(); this->_nbLine++) {
    ...
    if (this->_tokensVector[this->_nbLine].type == TK_SERVER) {
      ...
      this->_serversVector.push_back(Server(this->_serversVector,
                                            this->_tempServerConfigMap,
                                            this->_tempLocationMapVector));
