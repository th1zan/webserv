...
void Service::_readDataFromClient() {
  ...
  // the buffer is filled with the content passing to the listeningSocketFd then
  // the content is "read" with recv()
  int bytes =
      recv(this->_tmpServiceInfo.listeningSocketFd, buffer, BUFFER_SIZE, 0);

  if (bytes > 0) {
    this->_clientVector.at(this->_tmpServiceInfo.clientID)
        .appendRequest(buffer, bytes);
